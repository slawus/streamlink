...
RE_LOG_HEADER = r"(?:^|\n+)([Ss]treamlink\s+(\d+\.\d+\.\d+(?:-\S+)?)\s+\(\d{4}-\d{2}-\d{2}\)\n(?:-|=){3,})\n+"
...
try:
    ...
    changelogs = split(RE_LOG_HEADER, contents)[1:]
    changelogs = {v: "{0}\n\n{1}".format(changelogs[i-1], changelogs[i+1]) for i, v in enumerate(changelogs) if i % 3 == 1}
