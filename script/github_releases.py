...
def main(repo, tag, assets, api_key, dry_run=False):
    def github_api_call(host="api.github.com", method="GET", endpoint="/", raise_failure=True, **kwargs):
        ...
        response = req(url, headers=headers, **kwargs)
    ...
    def get_upload_url(response):
        return RE_HYPERMEDIA.match(get_response_json_key(response, "upload_url")).group(1)
    ...
    def get_file_handles():
        ...
        for asset in assets or []:
            filepath = path.join(root, asset)
            filename = path.basename(filepath)
            if not path.isfile(filepath):
                raise ValueError("File not found: {}", asset)
            handles[filename] = open(filepath, "rb")
    ...
    try:
        ...
        for filename, filehandle in filehandles.items():
            if dry_run:
                ...
                github_api_call(
                    host=upload_url or "uploads.github.com",
                    method="POST",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--asset",
        help="Upload a new asset to the Github release",
        action="append"
    )
    parser.add_argument(
        "--api-key",
        help="The Github API key, by default uses env.RELEASES_API_KEY",
    ...
    if args.repo and args.tag and (args.api_key or args.dry_run):
        exit(main(args.repo, args.tag, args.asset, args.api_key, args.dry_run))
    else:
